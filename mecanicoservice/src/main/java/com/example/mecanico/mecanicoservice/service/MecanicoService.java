...
public class MecanicoService {
    ...
    public MecanicoModel obtenerPorId(Long id) {
        ...
        return mecanicoRepository.findById(id)
                .orElseThrow(() ->
                new ResponseStatusException(HttpStatus.NOT_FOUND, "Mecánico no encontrado"));
    }
    ...
    public MecanicoResponseDTO actualizar(Long id, MecanicoRequestDTO request) {
        ...
        if (!existente.getRut().equals(request.getRut()) && mecanicoRepository.existsByRut(request.getRut())) {
            throw new ResponseStatusException(HttpStatus.CONFLICT, "El rut ya existe");
        }
