...
public class VehiculoService {
    ...
    public VehiculoResponseDTO registrarVehiculo(VehiculoRequestDTO request) {
        ...
        if (vehiculoRepository.existsByPatente(request.getPatente().toUpperCase())) {
            throw new ResponseStatusException(HttpStatus.CONFLICT, "La patente '" + request.getPatente() + "' ya está registrada.");
        }
        ...
        try {
            ...
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "El cliente con ID " + request.getClienteId() + " no existe en el sistema.");
        } catch (FeignException e) {
            throw new ResponseStatusException(HttpStatus.BAD_GATEWAY, "No fue posible validar el cliente externo.", e);
        } catch (Exception e) {
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "Ocurrió un error inesperado al registrar el vehículo.", e);
        }
